  ...
  const dropzone = rootUseDropzone({
    ...
    onDropAccepted: async (newFiles) => {
      ...
      if (maxNewFiles < newFiles.length) {
        if (shiftOnMaxFiles === true) { /* empty */ } else {
  ...
  ({ className, children, ...props }, forwardedRef) => {
    ...
    return (
      ...
      <div
        ...
        aria-label="dropzone"
        className={cn(
          'flex items-center justify-center rounded-md border border-zinc-200 bg-white p-0 ring-offset-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-950 focus-visible:ring-offset-2 dark:border-zinc-800 dark:bg-zinc-950 dark:ring-offset-zinc-950 dark:focus-visible:ring-zinc-300',
          context.isDragActive && 'animate-pulse bg-black/5',
          context.isInvalid && 'border-red-500 dark:border-red-900',
          className,
        )}
      >
        {children}
...
>((props, ref) => {
  ...
  return (
    <p
      ...
      {...rest}
      className={cn('pb-1 text-sm text-zinc-500 dark:text-zinc-400', className)}
    />
  );
});
  ...
  return (
    ...
    >
      <li
        ...
        className={cn(
          'flex flex-col justify-center gap-2 rounded-md bg-zinc-100/40 px-4 py-2 dark:bg-zinc-800/40',
          className,
        )}
      >
        {props.children}
...
>((props, ref) => {
  ...
  return (
    <p
      ...
      {...rest}
      className={cn(
        'h-5 text-[0.8rem] font-medium text-red-500 dark:text-red-900',
        rest.className,
      )}
    >
      {body}
...
const DropzoneTrigger = forwardRef<HTMLLabelElement, DropzoneTriggerProps>(
  ({ className, children, ...props }, ref) => {
    ...
    const { fileStatuses, getFileMessageId, isInvalid, rootMessageId } = context;
    ...
    return (
      <label
        ...
        {...props}
        className={cn(
          'cursor-pointer rounded-sm bg-zinc-100 px-4 py-2 font-medium ring-offset-white transition-colors focus-within:outline-none hover:bg-zinc-100/80 has-[input:focus-visible]:ring-2 has-[input:focus-visible]:ring-zinc-950 has-[input:focus-visible]:ring-offset-2 dark:bg-zinc-800 dark:ring-offset-zinc-950 dark:hover:bg-zinc-800/80 dark:has-[input:focus-visible]:ring-zinc-300',
          className,
        )}
      >
        {children}
        <input
          ...
          aria-describedby={
            isInvalid
              ? [rootMessageId, ...fileMessageIds].join('')
              : undefined
          }
    ...
    return (
      ...
      >
        <div
          ...
          className={cn(
            'h-full w-full rounded-full bg-zinc-900 dark:bg-zinc-50',
